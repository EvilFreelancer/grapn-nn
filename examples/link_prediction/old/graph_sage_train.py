...
# Создание меток связей
edge_labels = torch.zeros(len(node_list), len(node_list), dtype=torch.float)
for link in graph_data['links']:
    src = node_mapping[link['source']]
    tgt = node_mapping[link['target']]
    edge_labels[src][tgt] = 1
    edge_labels[tgt][src] = 1  # Если граф неориентированный
...
def train():
    ...
    loss = F.binary_cross_entropy_with_logits(out, data.edge_labels[data.train_mask])
    loss.backward()
    optimizer.step()
    return loss
