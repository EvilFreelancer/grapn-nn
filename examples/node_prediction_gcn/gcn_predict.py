...
# Load node to index and index to node mappings
with open('node_to_idx.json', 'r') as f:
    node_to_idx = json.load(f)
with open('idx_to_node.json', 'r') as f:
...
for _ in range(args.num_nodes):
    ...
    # Compute similarities between user's graph and large dataset
    similarities = cosine_similarity(user_output.detach().numpy(), output.detach().numpy())

    # Recommend a node
    recommended_node_index = np.argmax(similarities)
    recommended_node_name = idx_to_node[str(recommended_node_index)]
